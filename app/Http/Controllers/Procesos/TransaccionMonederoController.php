...
class TransaccionMonederoController extends Controller
{
    ...
    public function indexsolicitudretiro()
    {
        ...
        // WHERE sol.user_id=" . Auth()->user()->id;
        $sql = "SELECT sol.id,sol.user_id,sol.fecha,sol.cantidad,sol.rutaarchivo,sol.saldo,sol.aprobado,sol.transferido,sol.banco_benificiario,sol.tipo_cta,sol.num_cta_bancaria,tra.id transaccionid FROM solicitud_retiros sol
        LEFT JOIN transaccion_monedero tra on tra.solicitud_id=sol.id
        WHERE sol.user_id=" . Auth()->user()->id;
        $datos = DB::select($sql);
    ...
    public function solicitudretiro(Request $request)
    {
        ...
        if ($datos['cantidad'] > $misaldo[0]->saldo) {
            $request->session()->flash('alert-danger', 'Su saldo de $ ' . $misaldo[0]->saldo . ' no es suficiente  para realizar un retiro de la cuenta virtual.');
            return back();
        }
